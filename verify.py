...
def _run_pytest() -> tuple[StepResult, PytestSummary]:
    step = _run_command("Pytest", _python_cmd() + ["-m", "pytest", "-q"], ROOT)
    output = (step.stdout or "") + "\n" + (step.stderr or "")
    ...
    if permission_bug:
        step = StepResult(
            ...
            note="pytest teardown PermissionError treated as environment issue",
        )
...
def main() -> int:
    ...
    if api_step.skipped:
        success = len(hard_fail_steps) == 0 and pytest_step.success
    else:
        success = len(hard_fail_steps) == 0 and pytest_step.success and api_step.success
