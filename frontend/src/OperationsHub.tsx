...
function styles() {
    return {
        ...
        content: { flex: 1, minHeight: 0, overflow: "auto", padding: 12, display: "grid", gap: 10 } as CSSProperties,
        ...
        textarea: { width: "100%", minHeight: 96, resize: "vertical", boxSizing: "border-box", borderRadius: 10, border: "1px solid #3a3a3a", background: "#242424", color: "#ddd", padding: 10, outline: "none" } as CSSProperties,
    ...
    return (
        <aside style={st.shell}>
            <div style={st.header}>
                ...
                <div style={st.info}>
                    Workspace ativo e painel lateral redimensionável. Abra outra pasta pelo seletor de projetos sem reiniciar o backend.
                </div>
            ...
            <div style={st.content}>
                ...
                {section === "workspace" ? (
                    <>
                        ...
                        <div style={st.grid2}>
                            <Metric label="Confiança" value={fmtPct(selectedWorkspace?.classification.confidence ?? 0)} hint={selectedWorkspace?.classification.rationale ?? "Sem classificação"} />
                            <Metric label="Debate" value={selectedWorkspace?.decision.winner_role ?? "-"} hint={selectedWorkspace?.decision.recommended_tool ?? "sem decisão"} />
                        </div>

                        <div style={st.card}>
                            <div style={{ fontSize: 12, color: "#9ca3af" }}>Debate final</div>
                            <div style={{ marginTop: 6, fontSize: 12, lineHeight: 1.5, whiteSpace: "pre-wrap" }}>{selectedWorkspace?.debate_summary ?? "Sem debate ainda."}</div>
                            <div style={{ marginTop: 10 }}><ProgressBar value={selectedWorkspace?.classification.confidence ?? 0} /></div>
                        </div>

                        <div style={st.card}>
                            <div style={{ fontSize: 13, fontWeight: 800, color: "#fff" }}>Grafo de raciocínio</div>
                            <div style={{ marginTop: 10 }}>{graphNodes.length ? <GraphPreview nodes={graphNodes} /> : <div style={st.small}>Sem grafo disponível.</div>}</div>
                        </div>

                        <div style={st.card}>
                            <div style={{ fontSize: 13, fontWeight: 800, color: "#fff" }}>Última análise</div>
                            <div style={{ marginTop: 8, display: "grid", gap: 8 }}>
                                <div><span style={st.small}>Classificação: </span>{lastClassification ? JSON.stringify(lastClassification, null, 2) : "—"}</div>
                                <div><span style={st.small}>Decisão: </span>{lastDecision ? JSON.stringify(lastDecision, null, 2) : "—"}</div>
                                <div><span style={st.small}>Debate: </span>{lastDebate ? JSON.stringify(lastDebate, null, 2) : "—"}</div>
                            </div>
                        </div>
                    ...
                    <>
                        <div style={st.card}>
                            <div style={{ display: "flex", justifyContent: "space-between", gap: 8, alignItems: "center" }}>
                                ...
                                <StatusPill value={evolutionState?.enabled ? "running" : "stopped"} />
                            </div>
                            <div style={{ ...st.small, marginTop: 4 }}>Analisa o estado do projeto, gera backlog e aciona o próximo passo.</div>
                        </div>
                        ...
                        <div style={st.grid4}>
                            ...
                            <Metric label="Fila" value={evolutionState?.active_queue_size ?? 0} hint={`${evolutionState?.queued_items ?? 0} enfileirados`} />
                        ...
                        <div style={st.card}>
                            <div style={{ fontSize: 13, fontWeight: 800, color: "#fff" }}>Backlog priorizado</div>
                ...
                {section === "memory" ? (
                    <>
                        <div style={st.card}>
                            ...
                            <div style={{ ...st.small, marginTop: 4 }}>Decisões, padrões reutilizáveis e aprendizados acumulados.</div>
                        </div>

                        <div style={st.grid2}>
                            <Metric label="Aprendizados" value={cognitionLearning.length} hint="workspaces cognitivos" />
                            <Metric label="Memória" value={memory.length} hint={autonomyState?.recent_memory?.length ? "reutilizável" : "vazia"} />
                        </div>

                        <div style={st.card}>
                            <div style={{ fontSize: 13, fontWeight: 800, color: "#fff" }}>Aprendizado selecionado</div>
                            {selectedLearning ? (
                                <div style={{ marginTop: 10, display: "grid", gap: 8 }}>
                                    ...
                                    <div style={st.small}>{fmtDate(selectedLearning.timestamp)}</div>
                                    <div style={{ fontSize: 12, lineHeight: 1.45, whiteSpace: "pre-wrap" }}>{selectedLearning.summary}</div>
                                    <div style={st.small}>Ferramenta: {selectedLearning.recommended_tool} · Vencedor: {selectedLearning.winner_role} · Confiança: {fmtPct(selectedLearning.confidence)}</div>
                                </div>
                            ) : <div style={{ marginTop: 8, ...st.small }}>Sem aprendizado selecionado.</div>}
                        </div>
                ...
                {section === "control" ? (
                    <>
                        <div style={st.card}>
                            ...
                            <div style={{ ...st.small, marginTop: 4 }}>Crie objetivos, rode o autopilot, valide com verify e continue o ciclo.</div>
                        </div>
                        ...
                        <div style={st.card}>
                            ...
                            {verifyResult ? (
                                <pre style={{ marginTop: 10, marginBottom: 0, whiteSpace: "pre-wrap", fontSize: 11, lineHeight: 1.5, color: "#ddd" }}>{JSON.stringify(verifyResult, null, 2)}</pre>
                            ) : <div style={{ marginTop: 8, ...st.small }}>Sem resultados de verify ainda.</div>}
                        </div>

                        <div style={st.card}>
                            <div style={{ fontSize: 13, fontWeight: 800, color: "#fff" }}>Última análise</div>
                            <div style={{ marginTop: 10, display: "grid", gap: 8 }}>
                                <div><span style={st.small}>Classificação:</span><pre style={{ whiteSpace: "pre-wrap", margin: 0 }}>{lastClassification ? JSON.stringify(lastClassification, null, 2) : "—"}</pre></div>
                                <div><span style={st.small}>Decisão:</span><pre style={{ whiteSpace: "pre-wrap", margin: 0 }}>{lastDecision ? JSON.stringify(lastDecision, null, 2) : "—"}</pre></div>
                                <div><span style={st.small}>Debate:</span><pre style={{ whiteSpace: "pre-wrap", margin: 0 }}>{lastDebate ? JSON.stringify(lastDebate, null, 2) : "—"}</pre></div>
                            </div>
                        </div>
                    </>
                ) : null}
                ...
                {section === "integrations" ? (
                    <>
                        <div style={st.card}>
                            ...
                            <div style={{ ...st.small, marginTop: 4 }}>Composio e bridge alimentam a camada de decisão do ClawAI.</div>
                        </div>
