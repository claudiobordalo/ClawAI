...
function s() {
    return {
        shell: { width: 440, minWidth: 440, height: "100%", display: "flex", flexDirection: "column", background: "#171717", borderLeft: "1px solid #2d2d2d", color: "#ddd" } as CSSProperties,
        header: { padding: 12, borderBottom: "1px solid #2d2d2d", display: "grid", gap: 10 } as CSSProperties,
        ...
        content: { flex: 1, overflow: "auto", padding: 12, display: "grid", gap: 10 } as CSSProperties,
        ...
        textarea: { width: "100%", minHeight: 88, resize: "vertical", boxSizing: "border-box", borderRadius: 10, border: "1px solid #3a3a3a", background: "#242424", color: "#ddd", padding: 10, outline: "none" } as CSSProperties,
...
function Pill({ value }: { value: string }) {
    return <span style={s().pill}>{value}</span>;
}
...
function StatusPill({ value }: { value?: string | null }) {
    ...
    return <span style={{ ...s().pill, ...tone }}>{status}</span>;
}
...
export default function OperationsHub() {
    ...
    useEffect(() => { if (cognitionWorkspaces.length && !cognitionWorkspaces.some(ws => ws.workspace_id === selectedWorkspaceId)) setSelectedWorkspaceId(cognitionWorkspaces[0].workspace_id); }, [cognitionWorkspaces, selectedWorkspaceId]);
    useEffect(() => { if (cognitionLearning.length && !cognitionLearning.some(item => item.entry_id === selectedLearningId)) setSelectedLearningId(cognitionLearning[0].entry_id); }, [cognitionLearning, selectedLearningId]);
    ...
    async function queueObjective() {
        ...
        setControlError("");
        try {
            await enqueueAutonomy({ objective: text, test_command: testCommand.trim() || "uv run python -m pytest -q", max_iterations: maxIterations, max_files: maxFiles });
            setSection("control");
            await refreshAll();
        } catch (error) {
    ...
    async function startAutopilot() {
        ...
        setControlError("");
        try {
            const session = await startAutoImplement(text, testCommand.trim() || "uv run python -m pytest -q", maxIterations, maxFiles);
    ...
    async function runEvolutionAction(action: "start" | "stop" | "once" | "rebuild") {
        ...
        setControlError("");
        try {
            if (action === "start") await startEvolution();
            else if (action === "stop") await stopEvolution();
            else if (action === "once") await runEvolutionOnce();
            else await rebuildEvolutionBacklog();
            await refreshAll();
    ...
    async function runQuickClassify() {
        ...
        setTaskBusy(true);
        try {
            setLastClassification(await classifyCognition(text));
            setSection("control");
        } catch (error) {
    ...
    async function runQuickDebate() {
        ...
        setTaskBusy(true);
        try {
            const result = await consultBridge({ prompt: text, system_prompt: text });
    ...
    async function runQuickToolChoice() {
        ...
        setTaskBusy(true);
        try {
            setLastDecision(await recommendBridgeTool({ prompt: text, system_prompt: text }));
        ...
        <aside style={st.shell}>
            <div style={st.header}>
                <div style={st.titleRow}>
                    <div>
                        <div style={st.title}>ClawAI Studio</div>
                        <div style={st.subtitle}>Chat, workspace, evolução, memória e integrações em um só lugar</div>
                    </div>
                ...
                <div style={st.tabs}>
                    {SECTIONS.map(item => (
                        <button key={item.key} type="button" onClick={() => setSection(item.key)} style={st.tabButton(section === item.key)}>
                            <span style={st.tabLabel}>{item.label}</span>
                            <span style={st.tabHint}>{item.hint}</span>
                        </button>
                    ))}
                </div>
            </div>
            ...
            <div style={st.content}>
                {section === "chat" ? (
                    <>
                        <div style={st.card}>
                            ...
                            <div style={{ ...st.small, marginTop: 4, lineHeight: 1.45 }}>O chat ficou sempre visível e não depende de atalhos escondidos. Perguntas longas, instruções de implementação e análise passam por aqui.</div>
                        </div>
                        ...
                        <div style={st.card}>
                            ...
                            <div style={{ marginTop: 10 }}><textarea value={chatPrompt} onChange={e => setChatPrompt(e.target.value)} placeholder="Pergunte ao ClawAI..." style={st.textarea} /></div>
                            ...
                            {messages.map(message => (
                                <div key={message.id} style={st.message(message.role)}>
                                    ...
                                    {message.role === "assistant" && message.reply?.timings ? (
                                        <div style={{ marginTop: 8, ...st.small, lineHeight: 1.5 }}>
                ...
                {section === "workspace" ? (
                    <>
                        <div style={st.card}>
                            <div style={{ fontSize: 13, fontWeight: 800, color: "#fff" }}>Workspace cognitivo</div>
                            <div style={{ ...st.small, marginTop: 4, lineHeight: 1.45 }}>Esta seção mostra o espaço de raciocínio do ClawAI: workspaces, classificação, decisão e grafo de raciocínio. O sistema de arquivos do projeto continua no explorador à esquerda.</div>
                        </div>

                        <div style={st.card}>
                            <div style={{ display: "flex", justifyContent: "space-between", gap: 8, alignItems: "center" }}>
                                <div style={{ fontSize: 13, fontWeight: 800, color: "#fff" }}>Workspaces recentes</div>
                                <Pill value={selectedWorkspace?.classification.category ?? "workspace"} />
                            </div>
                            <div style={{ display: "grid", gap: 8, marginTop: 10 }}>
                                {cognitionWorkspaces.slice(0, 6).length ? cognitionWorkspaces.slice(0, 6).map(ws => (
                                    <button key={ws.workspace_id} type="button" onClick={() => setSelectedWorkspaceId(ws.workspace_id)} style={st.item(selectedWorkspaceId === ws.workspace_id)}>
                                        <div style={{ display: "flex", justifyContent: "space-between", gap: 8 }}><div style={{ fontWeight: 700, color: "#fff" }}>{ws.objective}</div><Pill value={ws.classification.category} /></div>
                                        <div style={{ marginTop: 4, ...st.small }}>Atualizado {fmtDate(ws.updated_at)}</div>
                                    </button>
                                )) : <div style={st.small}>Nenhum workspace encontrado ainda.</div>}
                            </div>
                        </div>
                        ...
                        {selectedWorkspace ? (
                            <div style={st.card}>
                                <div style={{ display: "flex", justifyContent: "space-between", gap: 8, alignItems: "center" }}>
                                    <div style={{ fontSize: 13, fontWeight: 800, color: "#fff" }}>{selectedWorkspace.objective}</div>
                                ...
                                <div style={{ marginTop: 10, display: "grid", gap: 8 }}>
                                    <div><div style={st.small}>Raciocínio</div><div style={{ marginTop: 4, fontSize: 12, lineHeight: 1.5 }}>{selectedWorkspace.classification.rationale}</div></div>
                                    <ProgressBar value={selectedWorkspace.classification.confidence} />
                                    <div><div style={st.small}>Debate final</div><div style={{ marginTop: 4, fontSize: 12, lineHeight: 1.5, whiteSpace: "pre-wrap" }}>{selectedWorkspace.debate_summary}</div></div>
                                    <div style={st.grid2}>
                                        <Metric label="Vencedor" value={selectedWorkspace.decision.winner_role} hint="agente eleito" />
                                        <Metric label="Confiança" value={fmtPct(selectedWorkspace.classification.confidence)} hint="do classificador" />
                                    </div>
                                    <div><div style={st.small}>Grafo de raciocínio</div><div style={{ marginTop: 6 }}>{graphNodes.length ? <GraphPreview nodes={graphNodes} /> : <div style={st.small}>Sem grafo disponível.</div>}</div></div>
                                </div>
                            </div>
                        ) : null}

                        <div style={st.card}>
                            <div style={{ fontSize: 13, fontWeight: 800, color: "#fff" }}>Plano persistente</div>
                            {latestPlan ? (
                                <div style={{ marginTop: 10, display: "grid", gap: 8 }}>
                                    <div style={{ fontWeight: 800, color: "#fff" }}>{latestPlan.objective}</div>
                                    <div style={st.small}>Plano {short(latestPlan.plan_id)} · Run {short(latestPlan.last_run_id)}</div>
                                    <div style={st.small}>Progresso {fmtPct(latestPlan.progress)} · passo {latestPlan.current_index}/{latestPlan.subtasks.length}</div>
                                    <ProgressBar value={latestPlan.progress} />
                                    <div style={{ display: "grid", gap: 6 }}>
                                        {latestPlan.subtasks.map((subtask, index) => (
                                            <div key={`${latestPlan.plan_id}-${index}`} style={st.card}>
                                                <div style={{ display: "flex", justifyContent: "space-between", gap: 8 }}><div style={{ fontWeight: 700, color: "#fff" }}>{subtask.title}</div><StatusPill value={subtask.status ?? "pending"} /></div>
                                                <div style={{ marginTop: 4, ...st.small }}>Progresso {fmtPct(subtask.progress ?? 0)}</div>
                                                {subtask.note ? <div style={{ marginTop: 4, fontSize: 11, lineHeight: 1.45, whiteSpace: "pre-wrap" }}>{subtask.note}</div> : null}
                                            </div>
                                        ))}
                                    </div>
                                </div>
                            ) : <div style={{ marginTop: 8, ...st.small }}>Nenhum plano persistente disponível.</div>}
                        </div>
                    </>
                ) : null}
                ...
                {section === "evolution" ? (
                    <>
                        <div style={st.card}>
                            <div style={{ fontSize: 13, fontWeight: 800, color: "#fff" }}>Evolution Engine</div>
                            <div style={{ ...st.small, marginTop: 4, lineHeight: 1.45 }}>Analisa o projeto, gera backlog, prioriza melhorias e dispara o próximo passo automaticamente.</div>
                        </div>
                        ...
                        <div style={st.grid4}>
                            ...
                            <Metric label="Prioridade" value={topBacklog ? `${topBacklog.priority}` : "-"} hint={topBacklog ? topBacklog.title : "sem backlog"} />
                        </div>

                        <div style={st.card}>
                            <div style={st.row}>
                                <button type="button" onClick={() => void runEvolutionAction("start")} disabled={taskBusy || activeEvolution} style={st.button(true)}>Start</button>
                                <button type="button" onClick={() => void runEvolutionAction("stop")} disabled={taskBusy || !activeEvolution} style={st.button()}>Stop</button>
                                <button type="button" onClick={() => void runEvolutionAction("once")} disabled={taskBusy} style={st.button()}>Run once</button>
                                <button type="button" onClick={() => void runEvolutionAction("rebuild")} disabled={taskBusy} style={st.button()}>Rebuild backlog</button>
                            </div>
                        </div>

                        <div style={st.card}>
                            <div style={{ fontSize: 13, fontWeight: 800, color: "#fff" }}>Melhorias priorizadas</div>
                            <div style={{ display: "grid", gap: 8, marginTop: 10 }}>
                                {evolutionBacklog?.items?.length ? evolutionBacklog.items.slice(0, 8).map(item => (
                                    <div key={item.backlog_id} style={st.card}>
                                        <div style={{ display: "flex", justifyContent: "space-between", gap: 8, alignItems: "center" }}><div style={{ fontWeight: 700, color: "#fff" }}>{item.title}</div><Pill value={`${item.priority}`} /></div>
                                        <div style={{ marginTop: 4, ...st.small }}>{item.category} · {fmtStatus(item.status)}</div>
                                        <div style={{ marginTop: 6, fontSize: 12, lineHeight: 1.45 }}>{item.description}</div>
                                        {item.reasons.length ? <div style={{ marginTop: 6, ...st.small }}>Motivos: {item.reasons.join(" · ")}</div> : null}
                                    </div>
                                )) : <div style={st.small}>O backlog ainda não foi gerado.</div>}
                            </div>
                        </div>

                        <div style={st.card}>
                            <div style={{ fontSize: 13, fontWeight: 800, color: "#fff" }}>Ciclos recentes</div>
                            <div style={{ display: "grid", gap: 8, marginTop: 10 }}>
                                {recentHistory.length ? recentHistory.map(record => (
                                    <div key={record.cycle_id} style={st.card}>
                                        <div style={{ display: "flex", justifyContent: "space-between", gap: 8, alignItems: "center" }}><div style={{ fontWeight: 700, color: "#fff" }}>Cycle {short(record.cycle_id)}</div><StatusPill value={record.status} /></div>
                                        <div style={{ marginTop: 4, ...st.small }}>{fmtDate(record.started_at)}</div>
                                        <div style={{ marginTop: 6, fontSize: 12, lineHeight: 1.45 }}>{record.analysis_summary}</div>
                                    </div>
                                )) : <div style={st.small}>Nenhum ciclo histórico ainda.</div>}
                            </div>
                        </div>
                    </>
                ) : null}
                ...
                {section === "memory" ? (
                    <>
                        <div style={st.card}>
                            <div style={{ fontSize: 13, fontWeight: 800, color: "#fff" }}>Memória e aprendizado</div>
                            <div style={{ ...st.small, marginTop: 4, lineHeight: 1.45 }}>Aqui ficam os aprendizados do sistema e o histórico de decisões que podem ser reutilizados nos próximos ciclos.</div>
                        </div>
                        ...
                        <div style={st.card}>
                            <div style={{ fontSize: 13, fontWeight: 800, color: "#fff" }}>Busca de aprendizado</div>
                            <div style={{ display: "grid", gap: 8, marginTop: 10 }}>
                                {selectedLearning ? (
                                    <div style={st.card}>
                                        <div style={{ display: "flex", justifyContent: "space-between", gap: 8 }}><div style={{ fontWeight: 700, color: "#fff" }}>{selectedLearning.objective}</div><Pill value={selectedLearning.category} /></div>
                                        <div style={{ marginTop: 4, ...st.small }}>{fmtDate(selectedLearning.timestamp)}</div>
                                        <div style={{ marginTop: 6, fontSize: 12, lineHeight: 1.45, whiteSpace: "pre-wrap" }}>{selectedLearning.summary}</div>
                                        <div style={{ marginTop: 6, ...st.small }}>Ferramenta: {selectedLearning.recommended_tool} · Vencedor: {selectedLearning.winner_role} · Confiança: {fmtPct(selectedLearning.confidence)}</div>
                                    </div>
                                ) : null}
                                <div style={{ display: "grid", gap: 8 }}>
                                    {cognitionLearning.slice(0, 8).map(item => (
                                        <button key={item.entry_id} type="button" onClick={() => setSelectedLearningId(item.entry_id)} style={st.item(selectedLearningId === item.entry_id)}>
                                            <div style={{ display: "flex", justifyContent: "space-between", gap: 8 }}><div style={{ fontWeight: 700, color: "#fff" }}>{item.objective}</div><Pill value={item.recommended_tool} /></div>
                                            <div style={{ marginTop: 4, ...st.small }}>{item.summary}</div>
                                        </button>
                                    ))}
                                </div>
                            </div>
                        ...
                        <div style={st.card}>
                            <div style={{ fontSize: 13, fontWeight: 800, color: "#fff" }}>Memória recente do sistema</div>
                            <div style={{ display: "grid", gap: 8, marginTop: 10 }}>
                                {memory.slice(0, 5).map(item => (
                                    <div key={item.memory_id} style={st.card}>
                                        <div style={{ display: "flex", justifyContent: "space-between", gap: 8 }}><div style={{ fontWeight: 700, color: "#fff" }}>{item.objective}</div><Pill value={item.outcome} /></div>
                                        <div style={{ marginTop: 4, ...st.small }}>{fmtDate(item.timestamp)}</div>
                ...
                {section === "control" ? (
                    <>
                        <div style={st.card}>
                            <div style={{ fontSize: 13, fontWeight: 800, color: "#fff" }}>Centro de controle</div>
                            <div style={{ ...st.small, marginTop: 4, lineHeight: 1.45 }}>Crie objetivos, rode o autopilot, valide com verify e deixe o ClawAI continuar o ciclo.</div>
                        </div>

                        <div style={st.card}>
                            <div style={{ display: "grid", gap: 8 }}>
                                <textarea value={objective} onChange={e => setObjective(e.target.value)} placeholder="Objetivo principal" style={st.textarea} />
                                <div style={st.grid3}>
                                    <input value={testCommand} onChange={e => setTestCommand(e.target.value)} placeholder="Comando de teste" style={st.input} />
                                    <input type="number" min={1} max={5} value={maxIterations} onChange={e => setMaxIterations(Number(e.target.value) || 1)} style={st.input} />
                                    <input type="number" min={1} max={20} value={maxFiles} onChange={e => setMaxFiles(Number(e.target.value) || 1)} style={st.input} />
                                </div>
                                <div style={st.grid3}>
                                    <button type="button" onClick={() => void queueObjective()} disabled={taskBusy || !objective.trim()} style={st.button(true)}>Enfileirar</button>
                                    <button type="button" onClick={() => void startAutopilot()} disabled={taskBusy || !objective.trim()} style={st.button(true)}>Autopilot</button>
                                    <button type="button" onClick={() => void runVerifyProject()} disabled={taskBusy} style={st.button()}>Verify</button>
                                </div>
                                <div style={st.grid2}>
                                    <button type="button" onClick={() => void runQuickClassify()} disabled={taskBusy || !objective.trim()} style={st.button()}>Classificar</button>
                                    <button type="button" onClick={() => void runQuickSupervision()} disabled={taskBusy || !objective.trim()} style={st.button()}>Supervisionar</button>
                                </div>
                                <details style={st.info}>
                                    <summary style={{ cursor: "pointer", fontWeight: 700, color: "#fff" }}>Ações avançadas</summary>
                                    <div style={{ marginTop: 10, display: "grid", gap: 8 }}>
                                        <div style={st.grid2}>
                                            <button type="button" onClick={() => void runQuickDebate()} disabled={taskBusy || !objective.trim()} style={st.button()}>Debate</button>
                                            <button type="button" onClick={() => void runQuickToolChoice()} disabled={taskBusy || !objective.trim()} style={st.button()}>Ferramenta</button>
                                        </div>
                                    </div>
                                </details>
                                {controlError ? <div style={{ ...st.info, borderLeftColor: "#f59e0b", background: "#2d2111", color: "#fee2b3" }}>{controlError}</div> : null}
                            </div>
                        </div>

                        <div style={st.card}>
                            <div style={{ fontSize: 13, fontWeight: 800, color: "#fff" }}>Sessão direta</div>
                            {autoSession ? (
                                <div style={{ marginTop: 10, display: "grid", gap: 6 }}>
                                    ...
                                    <div style={st.row}><button type="button" onClick={() => void cancelAutopilot()} disabled={taskBusy} style={st.button(false, true)}>Cancelar</button></div>
                                </div>
                            ) : <div style={{ marginTop: 8, ...st.small }}>Nenhuma sessão direta iniciada.</div>}
                        </div>

                        <div style={st.card}>
                            <div style={{ fontSize: 13, fontWeight: 800, color: "#fff" }}>Verificação</div>
                            {verifyResult ? <pre style={{ marginTop: 10, marginBottom: 0, whiteSpace: "pre-wrap", fontSize: 11, lineHeight: 1.5, color: "#ddd" }}>{JSON.stringify(verifyResult, null, 2)}</pre> : <div style={{ marginTop: 8, ...st.small }}>Sem resultados de verify ainda.</div>}
                        </div>
                    </>
                ) : null}
                ...
                {section === "integrations" ? (
                    <>
                        <div style={st.card}>
                            <div style={{ fontSize: 13, fontWeight: 800, color: "#fff" }}>Integrações e providers</div>
                            <div style={{ ...st.small, marginTop: 4, lineHeight: 1.45 }}>Composio e o bridge entre modelos alimentam a camada de decisão do ClawAI. Aqui fica visível o que está disponível para o motor cognitivo.</div>
                        </div>
                        ...
                        <div style={st.card}>
                            <div style={{ fontSize: 13, fontWeight: 800, color: "#fff" }}>Providers</div>
                            <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginTop: 10 }}>
                                {bridgeProviders?.providers.length ? bridgeProviders.providers.map(provider => <Pill key={provider} value={provider} />) : <div style={st.small}>Nenhum provider detectado.</div>}
                            </div>
                        </div>

                        <div style={st.card}>
                            <div style={{ fontSize: 13, fontWeight: 800, color: "#fff" }}>Ferramentas</div>
                            <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginTop: 10 }}>
                                {bridgeProviders?.tools.length ? bridgeProviders.tools.map(tool => <Pill key={tool} value={tool} />) : <div style={st.small}>Nenhuma ferramenta detectada.</div>}
                            </div>
                        </div>

                        <div style={st.card}>
                            <div style={{ fontSize: 13, fontWeight: 800, color: "#fff" }}>Workspaces cognitivos</div>
                            <div style={{ display: "grid", gap: 8, marginTop: 10 }}>
                                {cognitionWorkspaces.slice(0, 6).length ? cognitionWorkspaces.slice(0, 6).map(ws => (
                                    <button key={ws.workspace_id} type="button" onClick={() => setSection("workspace")} style={st.item(selectedWorkspaceId === ws.workspace_id)}>
                                        <div style={{ display: "flex", justifyContent: "space-between", gap: 8 }}><div style={{ fontWeight: 700, color: "#fff" }}>{ws.objective}</div><Pill value={ws.classification.category} /></div>
                                        <div style={{ marginTop: 4, ...st.small }}>Atualizado {fmtDate(ws.updated_at)}</div>
                                    </button>
                                )) : <div style={st.small}>Ainda sem workspaces.</div>}
                            </div>
                        </div>
                    </>
                ) : null}
